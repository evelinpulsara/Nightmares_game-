...
public class BulletController : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        hitboxMask = LayerMask.GetMask("Hitbox", "Enemy");
    }
    ...
    public void DetectCollision()
    {
        ...
        if (Physics.Raycast(lastBulletPos, bulletDirection.normalized, out hit, dist, hitboxMask.value))
        {
            ...
            BodyPartHitCheck part = go.GetComponent<BodyPartHitCheck>();
            if (part != null)
            {
                part.TakeHit(bulletDamage);
            }
            else
            {
                DopplegangerController enemy = go.GetComponent<DopplegangerController>();
                if (enemy == null) enemy = go.GetComponentInParent<DopplegangerController>();
                if (enemy != null)
                {
